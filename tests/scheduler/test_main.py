...
def windowed(broker: Broker, store: Store) -> App:
    """The scheduler this suite runs, given a window nothing will end early:
    the broker holds no `facing` row, so it waits the whole of one."""
...
def test_a_stop_inside_the_retained_window_is_acted_on_there(
    ...
    """The scheduler held to the rule every app comes up through, which is
