...
def test_the_drawings_are_listed(store: AssetStore) -> None:
    ...
    assert status == 200
    assert "gotthard" in body["drawings"]
...
def test_a_put_saves_the_drawing_and_keeps_its_prose(
    ...
    doc = store.drawing("gotthard")
    doc["symbols"]["sw16"]["at"] = [4, 7]
    status, _ = handle(store, "PUT", "/drawings/gotthard", doc)

    assert status == 200
    text = (tmp_path / "layouts" / "gotthard.drawing.yaml").read_text()
    assert "# The WX310, west of the station" in text
    assert store.drawing("gotthard")["symbols"]["sw16"]["at"] == [4, 7]
...
def test_an_unknown_route_is_not_found(store: AssetStore) -> None:
    for method, path in (("GET", "/nowhere"), ("DELETE", "/drawings/gotthard")):
