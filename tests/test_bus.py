...
def test_publish_queues_without_delivering() -> None:
    bus = Bus()
    seen: list[str] = []
    bus.subscribe("tc49/layout/tick", lambda topic, payload: seen.append(topic))
...
def test_publish_inside_handler_joins_back_of_queue() -> None:
    ...
    def on_tick(topic: str, payload: dict[str, object]) -> None:
        seen.append(topic)
        bus.publish("tc49/dispatch/move_granted", {"id": "t-1"})

    bus.subscribe("tc49/layout/tick", on_tick)
...
def test_filter_grammar() -> None:
    ...
    bus.subscribe(
        "tc49/layout/tick", lambda topic, payload: seen["exact"].append(topic)
    )
    ...
    assert seen["exact"] == ["tc49/layout/tick"]
    assert seen["plus"] == ["tc49/layout/tick", "tc49/layout/block_occupied"]
...
@pytest.mark.parametrize("bad", ["tc49/#/tick", "tc49/lay#", "tc49/ten+", "+tc49/x"])
