...
| `device/link/<id>` | `id`, `link`: `up` \| `down`; `detail` *optional*; `build` *optional* | Whether the publisher can reach its own hardware, keyed by whatever the publisher calls itself, and what build the far end reports it is running — the identifier alone, never the banner it was read out of. | The websocket, with the heartbeat the server advertises in its `hello` (about 13.5 s by default). It cannot say whether JMRI reaches the command station: the JSON protocol exposes no connection status, though JMRI tracks one internally. That gap is the operator's to cover. | The connection to the port, and the last field of the status banner as the `build`: a release tag on this railroad's firmware, the commit it was built from on older ones. |
