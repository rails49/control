...
**Batch arrivals** — every request in at the start. Makespan is then drain time,
...
3. **Redraw** — while any train's first request is one no dispatcher could
   ...
   are each other's standing locks, stalling the run at once with track to
...
**Quiescence, exactly detected — not a timeout.** A batch run stops when
the simulator's event queue is empty: nothing is scheduled, so no train is
rolling and no sensor is coming ([SYSTEM.md](../SYSTEM.md#layout-interface)).
That is exact, not heuristic: every request went in at the start, the
dispatcher is deterministic and event-driven, and a sweep runs wherever a
lock frees — no events means no change, forever. The stop rule is
...
policy bug. The simulator's event budget survives only as a backstop against
a live-lock bug, never as the normal stop condition.
...
delays, never the wall clock — and the determinism
