...
its junctions `station_a` still loads and still derives; **opening it replaces
...
junctions, wire the neighbours together, and derivation answers that `station_a`
and `station_c_west` are one connection. The editor cannot settle that, because
choosing which half is station-A is not its decision. The clash, the canvas tint
...
says `'station_c_2' is already taken` where the name was typed. Today the dialog
...
`reversing-loops`'s turnouts are named for RocRail's switch ids, deliberately,
and the drawing says so. Those names survive in the yaml and in the netlist
pane, but stop being visible on the canvas or editable in the dialog, and a
turnout drawn from now on gets `sw7` and means nothing by it. The tie to
hardware is `addr` instead, which is the tie that was wanted.

Its junctions stop being called `station_a`, `butterfly` and `station_c_west`
once the drawing is opened and saved. That is the point rather than a side
effect, and the names remain in the file's comments and in the netlist pane's
section headers under their minted replacements. If reading them back matters
more than expected, the answer is a display name on a junction, not a return to
typing the key.
