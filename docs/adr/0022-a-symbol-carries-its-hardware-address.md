...
Two things below have changed. "`addr` does not enter the layout document" is
...
for its positions. "Nothing checks it" stood, then did not, and stands again:
[ADR-0043](0043-the-layout-interface-is-a-core-app-and-hardware-hangs-under-it-by-address.md)
had an address name its system as its first level and derivation refuse one
that did not, and
[ADR-0059](0059-the-bus-is-a-broker-each-app-is-its-own-process-and-the-bridge-is-deleted.md)
withdrew that level. Everything else stands.
