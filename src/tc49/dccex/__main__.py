...
The startup order is `lib/startup.py`'s, and it has no store in it. **No
documents**: an app that reads nothing cannot be brought up in the wrong order
with respect to one that serves documents. So the broker is the first wait,
and `DccEx` on it — the constructor states this app's two opening rows,
`device/link/<id>: down` and a dark `device/track` carrying why, and a publish
made to a broker that is not there is dropped rather than queued (ADR-0050).

**No rows of a previous process to adopt**, and none of this app's own to read
back: the two it owns are stated by the constructor. What it waits for in
their place is the *desired* picture, which is `layout`'s to write and the
broker's to retain — and it is waited for **before the link is opened**, so
that the whole of it is held when the first connection is handed it. That is
what keeps the track row first: `_applied()` orders a connect's picture, where
a value arriving over a link that is already up is acted on as it arrives, and
a speed reaching the station ahead of the power is a locomotive that rolls the
moment somebody makes the rails live (#333, ADR-0054).
