...
describe("opening a drawing that names its junctions", () => {
  /** A throat wearing one name, and the review that says the two turnouts are
   *  one junction. `wearing` is what the file was written with. */
  async function opening(shell: TcEditor, wearing: string): Promise<void> {
    const typed: Drawing = {
      drawing: "gotthard",
      symbols: {
  ...
  it("replaces the name a person typed with a minted one", async () => {
    ...
    await opening(shell, "airolo");

    const { symbols } = session(shell).drawing;
    expect(symbols.sw1!.connection).toBe("j1");
    expect(symbols.sw2!.connection).toBe("j1");
  });
